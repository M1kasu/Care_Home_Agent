...
| 功能 | 状态 | 证据或原因 |
|---|---|---|
| Home Assistant 控制链路 | EXTERNAL_PASS | `reports/external/external_20260716_081632/` |
...
| 四类设备手动控制 | EXTERNAL_PASS | 灯光、开关、窗帘、空调均经 HA REST -> MQTT -> 回读 |
| UI 工作台 API | EXTERNAL_PASS | 独立进程黑盒调用设备、场景、分析、确认、故障和事件接口 |
| UI 浏览器体验 | BROWSER_PASS | 1440x900 与 390x844 无溢出/裁切，添加、控制、删除和主动执行成功 |
| 真实家庭硬件 | NOT_VERIFIED | 当前外部边界使用独立 MQTT 设备模拟器 |
| 真实无人感知 | NOT_VERIFIED | 当前客厅占用状态由 HA 演示实体注入，尚未接入毫米波/PIR |
